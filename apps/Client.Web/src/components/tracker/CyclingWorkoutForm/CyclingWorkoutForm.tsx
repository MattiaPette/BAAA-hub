...
// Maximum theoretical heart rate based on age (220 - age is common formula)
const MAX_THEORETICAL_HEART_RATE = 220;
...
export const CyclingWorkoutForm: FC<CyclingWorkoutFormProps> = ({
  ...
  // Sync state with prop changes (e.g., when editing a workout)
  useEffect(() => {
    if (value !== undefined) {
      setDistance(value.distance);
      setAverageSpeed(value.averageSpeed);
      setElevationGain(value.elevationGain);
      setAverageHeartRate(value.averageHeartRate);
  ...
  const updateDetails = useCallback(
    (updates: Readonly<Partial<CyclingWorkoutDetails>>) => {
    ...
    [distance, averageSpeed, elevationGain, averageHeartRate, notes, onChange],
  );
